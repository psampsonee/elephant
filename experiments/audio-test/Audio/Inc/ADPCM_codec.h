...
namespace ima_adpcm {
    ...
    static constexpr size_t FRAME_SAMPLE_COUNT =
    1 + (FRAME_SIZE - FRAME_HEADER_SIZE) * 2;

    typedef struct ima_adpcm_frameheader {
        int16_t iSample;           // Initial sample
        uint8_t iIndex;            // Initial index
        uint8_t reserved;          // Unused.
    } IMA_ADPCM_FRAMEHEADER;

    struct decompressed_pcm_frame {
        int16_t* samples;
        size_t sampleCount = FRAME_SAMPLE_COUNT;
    };
    ...
    // Helper functions
    IMA_ADPCM_FRAMEHEADER getFrameHeader(uint8_t* frame);
