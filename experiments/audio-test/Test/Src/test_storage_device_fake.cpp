...
constexpr size_t FAKE_SECTOR = 0;
...
void test_read_count_increments_on_read() {
    ...
    assert(h.storage.read(FAKE_SECTOR, buffer) == StorageDevice::ReadResult::Success);
    assert(h.storage.getReadCount() == 1);
    h.storage.read(FAKE_SECTOR, buffer);
...
void test_failure_functionality_read() {
    ...
    auto storageReadStatus = storage.read(FAKE_SECTOR, buffer);
...
void test_suppression_causes_read_not_ready() {
    ...
    auto storageReadStatus = h.storage.read(FAKE_SECTOR, buffer);
